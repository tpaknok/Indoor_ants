...
native.records <- native.records %>% filter(Final == "Yes") %>% filter(bentity2_name != "India" & bentity2_name != "Colombia")
exotic.records <- read.csv("Data/Exotic_record_date.csv")
exotic.records <- exotic.records %>% filter(Native_GABI == "N") %>% filter(Improved.Classification != "Native") %>% filter(bentity2_name != "India" & bentity2_name != "Colombia")
