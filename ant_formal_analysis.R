...
m <- glmmTMB(num~scaled.water_pca*scaled.niche_water_pca+scaled.temp_pca*scaled.niche_temp_pca+scaled.date+Generalist+Arboreal+(scaled.water_pca+scaled.temp_pca||species)+(scaled.niche_water_pca+scaled.niche_temp_pca||polygon_name),
...
p2b<- ggplot(predict_niche,aes(y=group,x=x))+
  ...
  scale_fill_continuous(low="blue",high="red",breaks=c(0.1,0.5,0.9))+
...
p2a <- ggplot(predict_water,aes(y=group,x=x))+
  ...
  scale_fill_continuous(low="blue",high="red",name="Predicted probability of establishing in the wild",breaks=c(0.1,0.5,0.9))+
...
future_df <- NMI_analysis[,c("species","polygon_name","shp.ID","sp.layer","Generalist","Arboreal","scaled.date","scaled.niche_temp_pca","scaled.niche_water_pca","dummy")]
...
predict_data <- list(NMI_analysis[,c("species","polygon_name","scaled.date","scaled.niche_temp_pca","scaled.niche_water_pca","scaled.temp_pca_2C","scaled.water_pca_2C","Arboreal","Generalist")],
                     NMI_analysis[,c("species","polygon_name","scaled.date","scaled.niche_temp_pca","scaled.niche_water_pca","scaled.temp_pca_4C","scaled.water_pca_4C","Arboreal","Generalist")])

predict_data <- lapply(predict_data,setNames,c("species","polygon_name","scaled.date","scaled.niche_temp_pca","scaled.niche_water_pca","scaled.temp_pca","scaled.water_pca","Arboreal","Generalist"))
...
p1a <- ggplot(data=bentity.df,aes(x=x,y=y,group=id,fill=indoor.sr))+
  ...
  labs(fill="")+
  scale_fill_continuous(low="blue",high="red",na.value="white",limits=c(0,71),breaks=c(1,35,70))+
  theme
...
p1c <- ggplot(data=bentity.df,aes(x=x,y=y,group=id,fill=indoor.sr.invasive))+
  ...
  annotation_custom(g1, xmin=-175, xmax=-175, ymin=-55, ymax=-55) +
  labs(fill="")+
  scale_fill_continuous(low="blue",high="red",na.value="white",limits=c(0.5,12.5),breaks=c(1,6,12))+
  theme
