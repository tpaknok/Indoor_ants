...
################## Get habitat of each invasive species
backup_NMI_analysis <- NMI_analysis
GISS <- read.csv("C:/Users/pakno/OneDrive - University of Toronto/Indoor_ants/GISS.csv")
...
GISS_long <- GISS %>% 
              ...
              summarize(max.score = max(score_confidence,na.rm=T),
                        min.score = min(score_confidence,na.rm=T))
...
site_impact_summary <- NMI_analysis %>% group_by(polygon_name,ID) %>% summarize(impact_total_current_E = sum(Impact_current.E.Total),
                                                                                ...
                                                                                current_invasive_indoor = sum(dummy[invasive=="Invasive" & num == 0])) %>% filter(current_invasive_indoor > 0)
...
mean(site_impact_summary$impact_total_2C_E)/mean(site_impact_summary$impact_total_current_E)*100
mean(site_impact_summary$impact_total_4C_E)/mean(site_impact_summary$impact_total_current_E)*100

mean(site_impact_summary$impact_total_2C_S)/mean(site_impact_summary$impact_total_current_S)*100
mean(site_impact_summary$impact_total_4C_S)/mean(site_impact_summary$impact_total_current_S)*100

##########################
Global_impact <- as_tibble(data.frame(polygon_name=NMI_analysis$polygon_name,num=NMI_analysis$num,Invasive=NMI_analysis$invasive,warm.4C=NMI_analysis$proj_diff_4C*Score_analysis,warm.2C=NMI_analysis$proj_diff_2C*Score_analysis)) %>%
                 group_by(polygon_name) %>%
                 filter(num==0 & Invasive == "Invasive") %>%
...
p5 <- ggplot(Global_impact,aes(label=n,y=Impact,x=Score,fill=Warming))+
...
ggsave("Indoor_ants/Total_impact.tiff",width=16,height=8,units="cm",dpi=600,compression="lzw")
##########################
...
plot(p1)
ggsave("Indoor_ants/Fig1.tiff",dpi=800,compression="lzw",units="cm",height=9.5*1.5,width=16.8,bg="white")

###

p3g <- ggplot(data=bentity.shp.sf,aes(fill=proj_diff_impact_E_2C_net))+
  geom_sf(colour="black",linewidth=0.1)+
  annotate("text", x = -Inf, y = Inf, hjust=0,vjust=1,label = "(e) Environmental",size=size)+
...
p3h <- ggplot(data=bentity.shp.sf,aes(fill=proj_diff_impact_E_4C_net))+
  geom_sf(colour="black",linewidth=0.1)+
...
p3i <- ggplot(data=bentity.shp.sf,aes(fill=proj_diff_impact_S_2C_net))+
  geom_sf(colour="black",linewidth=0.1)+
...
p3j <- ggplot(data=bentity.shp.sf,aes(fill=proj_diff_impact_S_4C_net))+
  geom_sf(colour="black",linewidth=0.1)+
...
p3k <- ggplot(data=bentity.shp.sf,aes(fill=warming_diff_impact_E))+
  geom_sf(colour="black",linewidth=0.1)+
...
p3l <- ggplot(data=bentity.shp.sf,aes(fill=warming_diff_impact_S))+
  geom_sf(colour="black",linewidth=0.1)+
...
p4 <- ggarrange(p3a,p3b,p3c,p3d,p3g,p3h,p3i,p3j,nrow=4,ncol=2)
plot(p4)
ggsave("Indoor_ants/Impact.tiff",dpi=800,compression="lzw",units="cm",height=13.5/3*4.5,width=16.8,bg="white")

#####################################################################
Species_summary <- NMI_analysis %>% group_by(species) %>% filter(invasive == "Invasive" & num == 0)  %>% summarize(increase_2C = sum(proj_diff_2C),
...
Species_p <- ggplot(data=Species_summary_long,aes(y=species,x=value,fill=Warming))+
  geom_bar(stat="Identity",position="dodge")+
  ...
  theme(legend.position="bottom",
        ...
        legend.title = element_text(size=6))
plot(Species_p)
ggsave("Indoor_ants/Species_p.tiff",dpi=800,compression="lzw",units="cm",height=13.5,width=16.8,bg="white")

###

p_warming_diff <- ggarrange(p3e,p3f,p3k,p3l)
plot(p_warming_diff)
ggsave("Indoor_ants/warming_diff.tiff",dpi=800,compression="lzw",units="cm",height=8.4*1.25,width=16.8,bg="white")
...
cor(subset(site_summary,current_indoor_invasive != 0)$proj_diff_invasive_indoor_2C_net,subset(site_summary,current_indoor_invasive != 0)$proj_diff_invasive_indoor_4C_net,method="kendall")
